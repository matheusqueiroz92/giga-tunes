# Boas-vindas ao repositório do projeto GigaTunes!

Aqui você vai encontrar os detalhes da estrutura e do desenvolvimento do projeto a partir deste repositório.

<details>
  <summary><strong>👨‍💻 O que foi desenvolvido</strong></summary><br />

  Neste projeto foi criado o GigaTunes, uma aplicação capaz de reproduzir músicas das mais variadas bandas e artistas, criar uma lista de músicas favoritas e editar o perfil da pessoa usuária logada.
...
<details>
  <summary><strong>Habilidades utilizadas neste projeto</strong></summary><br />

- Realizar requisições e consumir dados vindos de uma `API`;
...
<details>
  <summary><strong>Instruções para execução  deste projeto</strong></summary><br />

  1. Clone o repositório

  - Abra o terminal e digite o comando: `git clone git@github.com:matheusqueiroz92/giga-tunes.git` e aperte a tecla `enter`. Este processo copiará o diretório para a máquina local.
  - Entre na pasta do repositório que você acabou de clonar digitando o comando `cd giga-tunes` e aperte a tecla `enter`.

  2. Instale as dependências

  - Abra o terminal dentro da pasta do repositório e digite o comando `npm install` para instalar as depedências do projeto.
  
  3. Executando o projeto
  
  - Para executar a aplicação, após executar os passos anteriores, digite o comando `npm start` no terminal. Uma nova aba será aberta automaticamente no navegador e a aplicação será executada.
</details>

<details>
  <summary><strong>⌨️ Versionamentos do GitHub</strong></summary><br />
...
<details>
  ...
  e de fácil manutenção! Para rodá-los localmente no projeto, execute os comandos abaixo no terminal:
...
<details>
  ...
  Neste projeto foi utilizado a [React Testing Library (RTL)](https://testing-library.com/docs/react-testing-library/intro) para execução dos testes.
  ...
  É possível desabilitar temporariamente um teste utilizando a função `skip` junto à função `it`. Como o nome indica, esta função "pula" um teste:
...
<details>
  ...
  As funcionalidades desta aplicação serão agrupadas e organizadas em rotas.

  Uma rota define o que deve ser renderizado na página ao abrí-la. Cada rota está associada a um caminho. O caminho é a parte da URL após o domínio (nome do site, de forma simplificada). Por exemplo, em `www.site.com/projetos/meu-jogo`, o caminho é `/projetos/meu-jogo`.

  Esta aplicação irá consumir e enviar dados para uma API para pesquisar a banda ou artista, recuperar as músicas de cada álbum e salvar as músicas favoritas, além de editar as informações da pessoa logada. Dessa forma, serão realizadas requisições assíncronas e promises. Também foram feitos o uso dos métodos de ciclo de vida (lifecycle methods) e de estados para controlar o que é renderizado por seus componentes dependendo do momento em que as requisições se encontram.
